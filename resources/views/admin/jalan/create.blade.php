<x-app title="Tambah Jalan">
    <main class="app-main">
        <div class="app-content-header">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="mb-0">Tambah Jalan</h3>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-end">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('jalan.index') }}">Jalan</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Tambah Jalan</li>
                        </ol>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="card card-primary card-outline mb-4">
                            <div class="card-header">
                                <div class="card-title">Form Tambah Jalan</div>
                            </div>
                            <form action="{{ route('jalan.store') }}" method="POST" id="form-tambah-jalan">
                                @csrf
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <select name="titik_awal_id" id="titik_awal_id"
                                            ...
                                            @foreach ($titikJalan as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ old('titik_awal_id') == $item->id ? 'selected' : '' }}>
                                                    {{ $item->nama_titik }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="titik_akhir_id" id="titik_akhir_id"
                                            ...
                                            @foreach ($titikJalan as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ old('titik_akhir_id') == $item->id ? 'selected' : '' }}>
                                                    {{ $item->nama_titik }}
                                                </option>
                                            @endforeach
                                        </select>
    ...
    @push('scripts')
        <script type="module">
            document.addEventListener('DOMContentLoaded', function() {
                const form = document.getElementById('form-tambah-jalan');
                const titikAwal = document.getElementById('titik_awal_id');
                const titikAkhir = document.getElementById('titik_akhir_id');
                const existingJalan = {!! json_encode(\App\Models\Jalan::select('titik_awal_id', 'titik_akhir_id')->get()) !!};
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    if (!awalVal || !akhirVal) return; // biarkan required HTML5 jalan
                    ...
                    if (isDuplicate) {
                        ...
                        Swal.fire({
                            ...
                            text: 'Rute jalan dari titik tersebut ke titik akhir sudah pernah ditambahkan.',
                            confirmButtonColor: '#d33',
                        });
                    }
                });
