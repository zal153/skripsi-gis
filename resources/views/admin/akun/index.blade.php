                    ...
                    <div class="card mb-4">
                        <div
                            ...
                            <div class="d-flex w-100 w-md-auto align-items-center">
                                <form action="{{ route('akun.index') }}" method="GET" class="d-flex me-2 w-100"
                                    id="search-form">
                                    ...
                                    <div class="input-group input-group-sm">
                                        ...
                                        @if (request('search'))
                                            <a href="{{ route('akun.index', ['per_page' => request('per_page', 10)]) }}"
                                ...
                                <a href="{{ route('akun.create') }}" class="btn btn-sm btn-primary text-nowrap">Tambah
                                    Data</a>
                            </div>
                        </div>
                        <!-- /.card-header -->
                                ...
                                <table class="table table-bordered table-hover align-middle" id="akunTable">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th width="35%">
                                                <a href="{{ route('akun.index', ['sort_by' => 'name', 'sort_dir' => request('sort_by') == 'name' && request('sort_dir') == 'asc' ? 'desc' : 'asc', 'search' => request('search'), 'per_page' => request('per_page')]) }}"
                                                    class="text-decoration-none text-dark d-flex justify-content-between align-items-center">
                                                    Nama
                                                    @if (request('sort_by') == 'name')
                                                        <i
                                                            class="bi bi-sort-{{ request('sort_dir') == 'asc' ? 'up' : 'down' }}"></i>
                                                    @else
                                            ...
                                            <th width="40%">
                                                <a href="{{ route('akun.index', ['sort_by' => 'email', 'sort_dir' => request('sort_by') == 'email' && request('sort_dir') == 'asc' ? 'desc' : 'asc', 'search' => request('search'), 'per_page' => request('per_page')]) }}"
                                                    class="text-decoration-none text-dark d-flex justify-content-between align-items-center">
                                                    Email
